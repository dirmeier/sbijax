...
def run(name: str, seed: int, budget: dict, out: str) -> None:
  ...
  with tasks.Timer() as t:
    post = sabc.run(
      ...
      n_particles=budget["n_particles"], n_simulation=budget["n_simulation"],
      schedule=sabc.MultiEps(v=1.0), proposal=sabc.DiffEvolution(),
      key=mx.random.key(seed),
    )
    mx.eval(post.samples)
  tasks.save_result(out, "sabc-mlx", seed, np.asarray(post.samples), t.seconds, 0.0)
