...
def test_nle_objective_trains_and_samples_with_sampler():
  ...
  def sim(seed, theta):
    return theta["theta"] + tfd.Normal(0.0, 1.0).sample(theta["theta"].shape, seed=seed)
  obj = nle(make_maf(2))
  ...
  samples, info = sample(jr.key(2), obj, params, jnp.zeros(2),
                         sampler=sampler, n_chains=2, n_samples=40, n_warmup=20)
