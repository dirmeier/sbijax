...
plt.style.use("styles/sbijax.mplstyle")


def plot_trace(inference_data: az.InferenceData, axes: np.ndarray[pyplot.Axes]=None) -> pyplot.Axes:
    if axes is None:
        n_dim = len(inference_data.posterior.data_vars)
        _, axes = pyplot.subplots(nrows=n_dim, ncols=2)
    axes = az.plot_trace(
        inference_data,
        axes=axes.reshape(n_dim, 2)
    )
...
def plot_posterior(inference_data: az.InferenceData, axes: np.ndarray[pyplot.Axes]=None) -> pyplot.Axes:
    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    if axes is None:
        n_dim = np.sum(
            [v for k, v in dict(inference_data.posterior.sizes).items() if k not in ["chain", "draw"]]
        )
...
def plot_loss_profile(losses: jax.Array, axes: pyplot.Axes=None) -> pyplot.Axes:
    if axes is None:
...
def plot_rank(inference_data: az.InferenceData, axes: np.ndarray[pyplot.Axes]=None) -> pyplot.Axes:
    colors = plt.rcParams['axes.prop_cycle'].by_key()['color']
    ...
    if axes is None:
        ...
        max_dim = np.max(
            [v for k, v in dict(var_sizes).items() if
             k not in ["chain", "draw"]]
        )
...
def plot_ess(inference_data: az.InferenceData, axes: np.ndarray[pyplot.Axes]=None) -> pyplot.Axes:
    if axes is None:
        var_sizes = inference_data.posterior.sizes
        max_dim = np.max(
            [v for k, v in dict(var_sizes).items() if
...
def plot_rhat_and_ress(inference_data: az.InferenceData, axes: np.ndarray[pyplot.Axes]=None) -> pyplot.Axes:
