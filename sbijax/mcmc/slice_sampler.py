...
def kernel(n_doublings: int, doubling_procedure) -> Callable:
    if doubling_procedure == "tfp":
        db_fn = _tfp_doubling_fn
    else:
        db_fn = _doubling_fn

    def one_step(rng_key: PRNGKey, state: SliceState, logdensity_fn: Callable):
        proposal_generator = slice_proposal(logdensity_fn, n_doublings, db_fn)
...
class slice_sampler:
    ...
    def __new__(  # type: ignore[misc]
        ...
        n_doublings: int = 5,
        doubling_procedure="tfp",
    ) -> MCMCSamplingAlgorithm:
        step = cls.kernel(n_doublings, doubling_procedure)
...
def slice_proposal(logdensity_fn: Callable, n_doublings: int, db_fn):
...
def _sample_conditionally(
    seed, logdensity_fn, idx, positions, widths, n_doublings, db_fn
):
    ...
    left, right, _ = db_fn(seed1, y, x0, cond_lp_fn, w0, n_doublings)
...
def _tfp_doubling_fn(rng, y, x0, cond_lp_fn, w, n_doublings):
    """
    Implementation according to Fig 4 in [1]
    References
    -------
    ...
    left_oks = left_lps < y
    right_oks = right_lps < y
    both_ok = jnp.logical_and(left_oks, right_oks)
    # best_interval_idx = _best_interval(both_ok.astype(jnp.int32))
    best_interval_idx = jnp.argmin(both_ok.astype(jnp.int32))
