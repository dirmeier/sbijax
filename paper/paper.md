...
Models in sciences like astrophysics, biology or neuroscience can be frequently formulated as simulators functions, i.e., computer programs that stochastically map a parameter vector $\theta$ to synthetic data $x$ [@ulzega2025shedding; @ableidinger2017stochastic]. Even thouth simulators of this kind can represent realsitic descriptions of a real-world data-generating process, they often do not emit tractable probability density or mass functions. Bayesian inference for the parameters of a simulator requires the likelihood $p(x | \theta)$, which marginalizes over all latent variables that a simulation run draws and which is in general intractable, such that inference algorithms that rely on pointwise likelihood evaluations, such as Markov chain Monte Carlo [@brooks2011mcmc], cannot be applied.
...
A neural estimator, e.g., `nle(make_maf(2))`, returns training primitives and a sampling function which are consumed by the generic drivers `train` and `sample`, while an ABC method, e.g., `sabc(prior, simulator)`, returns a sampler that draws from the approximate posterior directly.
...
An inference workflow typically written as a short sequence of ordinary function calls, where the inputs and outputs of every step, e.g., the simulated data set and the network parameters, are managed by the developer explicitely.
This asks developer to be deliberate about the workflow they are executing rather than calling the member functions of an object whose internal state is (often) opaque to them.
We consider this trade-off worthwhile for a scientific software tool, since it makes a method transparent to the practitioner using it, albeit it induces additional complexity, easy to modify for a researcher extending it.
