...
In natural sciences like astrophysics, biology or neuroscience, models can be frequently formulated as simulators functions, i.e., computer programs that stochastically map a parameter vector $\theta$ to synthetic data $x$. Even thouth simulators of this kind can represent realistic descriptions of real-world data-generating processes, they often do not emit tractable probability density or mass functions. Statistical inference of the parameters of a simulator requires the likelihood $p(x | \theta)$, which marginalizes over all latent variables that a simulation run draws and which is in general intractable, such that inference algorithms that rely on pointwise likelihood evaluations, such as Markov chain Monte Carlo, cannot be applied.
...
A neural estimator, e.g., `nle(make_maf(\codt))`, returns training primitives and a sampling function which are consumed by the generic drivers `train` and `sample`, while an ABC method, e.g., `sabc(prior, simulator)`, returns a sampler that draws from the approximate posterior directly.
...
We consider this trade-off worthwhile for a scientific software tool, since it makes a method transparent to the practitioner using it (albeit it induces some additional complexity) and staightforward to modify for a researcher extending it.
